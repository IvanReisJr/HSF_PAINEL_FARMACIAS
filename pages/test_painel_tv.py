...
def fetch_lotes_tv():
    ...
    try:
        ...
        sql = oracle.load_sql("test_painel_tv_turno")
...
def render_painel():
    st.markdown("<h1 style='text-align: center;'>📺 (TESTE) Painel de Distribuição de Medicamentos</h1>", unsafe_allow_html=True)
    ...
    if not df.empty:
        ...
        st.markdown("---")
        st.markdown(f"### 📦 Cesta de Separação — Lote: **{nr_lote}** | Paciente: **{paciente_nome}**")
